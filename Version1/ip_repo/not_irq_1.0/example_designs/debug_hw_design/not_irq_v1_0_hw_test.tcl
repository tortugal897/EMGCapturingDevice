# Runtime Tcl commands to interact with - not_irq_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/not_irq_v1_0_include.tcl
...
# Test: in_irq
# Create a write transaction at in_irq_addr address
create_hw_axi_txn w_in_irq_addr [get_hw_axis $jtag_axi_master] -type write -address $in_irq_addr -data $wdata_1
# Create a read transaction at in_irq_addr address
create_hw_axi_txn r_in_irq_addr [get_hw_axis $jtag_axi_master] -type read -address $in_irq_addr
# Initiate transactions
run_hw_axi r_in_irq_addr
run_hw_axi w_in_irq_addr
run_hw_axi r_in_irq_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_in_irq_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - in_irq"
} else {
	puts "Data comparison test fail for - in_irq, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: not_irq
set wdata_not_irq [get_init_data 0]
create_hw_axi_txn w_not_irq_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_not_irq}
create_hw_axi_txn r_not_irq_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_not_irq_addr
run_hw_axi w_not_irq_addr
run_hw_axi r_not_irq_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_not_irq_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - not_irq"
} else {
	puts "Data comparison test fail for - not_irq, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
