...
#define ADS_CS_PIN 53 // CS pin is mandatory
#define ADS_DRDY_PIN 20 // DRDY pin is mandatory
...
#define ADS_RESET_PIN 47 // If it not used, it could be the constant ADS_PIN_NOT_USED or call the constructor with only two arguments
#define ADS_START_PIN 4
#define ADS_CLKSEL_PIN 49
#define CanalesAUtilizar 8
//unsigned long tiempo1 = 0;
//unsigned long tiempo2 = 0;

// Constructor
ADS129xSensor adsSensor(ADS_CS_PIN, ADS_DRDY_PIN);//, ADS_RESET_PIN, ADS_START_PIN); // Crea el objeto adsSensor de clase ADS129xSensor.
...
void setup() {
  ...
  for (uint8_t i = 1; i <= ADS_N_CHANNELS; i++)
    adsSensor.enableChannelAndSetGain(i, ads::registers::chnSet::GAIN_6X, ads::registers::chnSet::ELECTRODE_INPUT);

  //Para visualizar los valores que han tomado los registros después de la configuración de test signal.
  Serial.print("Valor registro WCT1: ");
  printBits(adsSensor.readRegister(ads::registers::wct1::REG_ADDR));
  Serial.println("");

  Serial.print("Valor registro WCT2: ");
  printBits(adsSensor.readRegister(ads::registers::wct2::REG_ADDR));
  Serial.println("");

  Serial.print("Valor registro config 2: ");
  printBits(adsSensor.readRegister(ads::registers::config2::REG_ADDR));
  Serial.println("");

  // Continuación de la secuencia de inicio.
  Serial.println("Starting channels configuration");
  ...
  //adsSensor.sendSPICommandRDATAC();  
...
void loop() {
  ...
  if (adsSensor.hasNewDataAvailable()) {
    ...
    *byteSample = (byte*) &adsData->formatedData.channel[1];
    mask = byteSample[3] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[3] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[4] << 8) + byteSample[5];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[2];
    mask = byteSample[6] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[6] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[7] << 8) + byteSample[7];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[3];
    mask = byteSample[8] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[8] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[9] << 8) + byteSample[10];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[4];
    mask = byteSample[11] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[11] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[12] << 8) + byteSample[13];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[5];
    mask = byteSample[14] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[14] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[15] << 8) + byteSample[16];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[6];
    mask = byteSample[17] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[17] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[18] << 8) + byteSample[19];
    Serial.print(sampleValue);
    Serial.print("\t");

    *byteSample = (byte*) &adsData->formatedData.channel[7];
    mask = byteSample[20] & 0x80;
    mask ^= 0x80;
    offsetBinary = mask | (byteSample[20] & 0x7F);
    sampleValue = (offsetBinary << 16) + (byteSample[21] << 8) + byteSample[22];
    Serial.print(sampleValue);
    /*
        // Print in binary
    ...
    */
    ...
    Serial.println("");
    adsSensor.sendSPICommandRDATA(); // Comentar en RDATAC
    //tiempo2 = millis();
